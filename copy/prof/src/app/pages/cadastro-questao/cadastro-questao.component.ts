...
export class CadastroQuestaoComponent implements OnInit {
  ...
  private montarFormulario(questao?: any | iQuestao): () => void {
    ...
    this.cadastroQuestao = this.fb.group({
      ...
      cdDificuldade: [questao.cdDificuldade ?? 1, Validators.required],
      cdCompetencia: [questao.cdCompetencia ?? 1, Validators.required],
      cdAreaConhecimento: [
        questao.cdAreaConhecimento ?? '',
        Validators.compose([
          Validators.required,
          this.CustomRequired(
      ...
      cdProfessorResponsavel: [
        questao.cdProfessorResponsavel ?? '',
        Validators.compose([
          Validators.required,
          this.CustomRequired(
            () => this.professores,
    ...
    let metodosCargas: () => void = () => {
      this.cadastroQuestao.controls.cdQuestaoTipo.valueChanges.subscribe(
        (cdQuestaoTipo) => this.setCdTipoQuestao(cdQuestaoTipo)
      );

      this.cadastroQuestao.controls.cdSerie.valueChanges.subscribe((cdSerie) =>
        this.serieChange(cdSerie)
      );

      this.cadastroQuestao.controls.cdAreaConhecimento.valueChanges.subscribe(
        (cdAreaConhecimento) => this.areaConhecimentoChange(cdAreaConhecimento)
      );

      this.cadastroQuestao.controls.cdDisciplina.valueChanges.subscribe(
        (cdDisciplina) => this.disciplinaChange(cdDisciplina)
      );

      this.cadastroQuestao.controls.cdTopico.valueChanges.subscribe(
        (cdTopico) => this.topicoChange(cdTopico)
      );

      // this.cadastroQuestao.controls.cdSubTopico.valueChanges.subscribe(
      //   (cdSubTopico) => this.subTopicoChange(cdSubTopico)
      // );
    };
  ...
  async ngOnInit() {
    ...
    await this.api
      .api(
        HttpMethod.GET,
        `competencia/empresa/${this.auth.currentUserValue.cdEmpresa}`
      )
      .then((res) => {
      ...
      .api(
        HttpMethod.GET,
        `habilidade/${this.auth.currentUserValue.cdEmpresa}`
      )
      .then((res) => {
        this.habilidades = res;
      });

    await this.api.api(HttpMethod.GET, 'questao/dificuldade').then((res) => {
    ...
    await this.api.api(HttpMethod.GET, 'questao/tipos/').then((res) => {
      this.tiposQuestao = res;

      if (this.tiposQuestao && this.tiposQuestao.length > 0) {
        this.tiposQuestao.push({
          cdQuestaoTipo: 1,
          nmQuestaoTipo: 'Redação',
          stRedacao: true,
        });
    ...
    this.route.paramMap.subscribe(async (param) => {
      if (!Number.isNaN(Number(param.get('cdQuestao')))) {
        ...
        await this.api
          .api(HttpMethod.GET, `questao/${cdQuestao}`)
          .then((res) => {
            questao = res;
          });
        ...
        if (questao.refSuporte != null)
          this.cadastroQuestao.controls.refSuporte.setValue(questao.refSuporte);

        if (questao.refComando != null)
          this.cadastroQuestao.controls.refComando.setValue(questao.refComando);

        if (
          questao.cdDificuldade == 0 ||
          this.dificuldades.findIndex(
            (x) => x.cdDificuldade == questao.cdDificuldade
          ) == -1
        )
          this.cadastroQuestao.controls.cdDificuldade.setValue('');
        else
          this.cadastroQuestao.controls.cdDificuldade.setValue(
            questao.cdDificuldade
          );

        if (
          questao.cdOrigem == 0 ||
          this.origens.findIndex((x) => x.cdOrigem == questao.cdOrigem) == -1
        )
          this.cadastroQuestao.controls.cdOrigem.setValue('');
        else this.cadastroQuestao.controls.cdOrigem.setValue(questao.cdOrigem);

        if (
          questao.cdCompetencia == 0 ||
          this.competencias.findIndex(
            (x) => x.cdCompetencia == questao.cdCompetencia
          ) == -1
        )
        ...
        if (
          questao.cdSerie == 0 ||
          this.series.findIndex((x) => x.cdSerie == questao.cdSerie) == -1
        )
          ...
          if (
            questao.cdAreaConhecimento == 0 ||
            this.areasConhecimento.findIndex(
              (x) => x.cdAreaConhecimento == questao.cdAreaConhecimento
            ) == -1
          )
            ...
            if (
              questao.cdDisciplina == 0 ||
              this.disciplinas.findIndex(
                (x) => x.cdDisciplina == questao.cdDisciplina
              ) == -1
            )
              ...
              if (
                ...
                this.topicos.findIndex(
                  (x) => questao.cdTopico.findIndex((y) => y == x.cdTopico) > -1
                ) == -1
              )
                ...
                if (
                  ...
                  this.subTopicos.findIndex(
                    (x) =>
                      questao.cdSubTopico.findIndex((y) => y == x.cdSubTopico) >
                      -1
                  ) == -1
                )
                  ...
                  if (
                    questao.cdHabilidade == 0 ||
                    this.habilidades.findIndex(
                      (x) => x.cdHabilidade == questao.cdHabilidade
                    ) == -1
                  )
        ...
        if (
          questao.cdProfessorResponsavel == 0 ||
          this.professores.findIndex(
            (x) => x.cdProfessor == questao.cdProfessorResponsavel
          ) == -1
        )
          this.cadastroQuestao.controls.cdProfessorResponsavel.setValue('');
        ...
        if (questao.cdQuestaoTipo > 1) {
          if (questao.cdQuestaoTipo == 3) {
            this.Alternativas.setValidators(
              Validators.compose([this.validarCorreta()])
            );
          ...
          if (alternativas && alternativas.length) {
            ...
            this.cadastroQuestao.addControl(
              'Alternativas',
              new FormArray([
                ...alternativas.map((x) =>
                  this.cdQuestaoTipo == 2 || this.cdQuestaoTipo == 3
                    ? this.fb.group({
                        cdQuestao: [
                          this.cadastroQuestao.controls.cdQuestao.value,
                        ],
  ...
  async areaConhecimentoChange(cdAreaConhecimento) {
    ...
    if (cdAreaConhecimento) {
      await this.api
        .api(
          HttpMethod.GET,
          `disciplina/areaconhecimento/${cdAreaConhecimento}/${this.cadastroQuestao.controls.cdSerie.value}/${this.auth.currentUserValue.cdEmpresa}`
        )
        .then((res) => {
    ...
    switch (this.cadastroQuestao.controls.cdQuestaoTipo.value) {
      ...
      case 3:
        ...
        if (index == this.Alternativas.length - 1 && val) {
          alternativa.controls.dsAlternativa1.setValidators(
            Validators.required
          );
        ...
        } else if (index == this.Alternativas.length - 2 && !val) {
      ...
      case 4:
        ...
        if (index == this.Alternativas.length - 1 && val && val2) {
          ...
          alternativa.controls.dsAlternativa2.setValidators(
            Validators.required
          );
        ...
        } else if (index == this.Alternativas.length - 2 && (!val || !val2)) {
          if (index > 0 && !val && !val2) {
            alternativa.controls.dsAlternativa1.clearValidators();
            alternativa.controls.dsAlternativa1.setErrors(null);
  ...
  changeEditorType(index: number) {
    ...
    //Reciclava quando retornava de html para texto normal, foi comentado por necessidade de "não" perder o tópico já inserido nos campos
    if (alternativa.controls.imgAlternativa1.value) {
      if (/<\/?[a-z][\s\S]*>/.test(alternativa.controls.dsAlternativa1.value)) {
        alternativa.controls.dsAlternativa1.setValue(
          $(alternativa.controls.dsAlternativa1.value).text()
        );
      }
    }
    alternativa.controls.imgAlternativa1.setValue(
      !alternativa.controls.imgAlternativa1.value
    );
  }
  ...
  changeEditor2Type(index: number) {
    ...
    if (alternativa.controls.imgAlternativa2.value) {
      if (/<\/?[a-z][\s\S]*>/.test(alternativa.controls.dsAlternativa2.value)) {
        alternativa.controls.dsAlternativa2.setValue(
          $(alternativa.controls.dsAlternativa2.value).text()
        );
      }
    }

    alternativa.controls.imgAlternativa2.setValue(
      !alternativa.controls.imgAlternativa2.value
    );
  }
  ...
  changeQuestao(cdQuestaoTipo) {
    ...
    if (cdQuestaoTipo == this.tiposQuestao.length + 1)
      this.cadastroQuestao.controls.stRedacao.setValue(true);
    else this.cadastroQuestao.controls.stRedacao.setValue(false);
  }
  ...
  Salvar(value: iQuestao) {
    ...
    if (this.cadastroQuestao.valid) {
      ...
      this.isSaving = true;
      this.api
        .api(HttpMethod.POST, `questao`, value)
        .then(async (res) => {
          if (!res)
  ...
  async confirmacaoSalvar(): Promise<boolean> {
    let cdQuestaoNova = 0;
    await this.api.api(HttpMethod.GET, `questao/getLastId`).then((res) => {
      cdQuestaoNova = res;
    });
